...
os.chdir(sys.path[0])
pattern = r"https://prts.wiki/images/[0-9a-f]{1,}/[0-9a-f]{1,}/.*?image_process=3Dformat,webp/quality,Q_90"
seen_values = set()
...
def extract_data(file_path):
    ...
    urls = re.findall(pattern, mhtml_content)
    src_data_unique = {}

    for idx, url in enumerate(urls, start=1):
        src_data_unique[f"src_{idx}"] = str(url).removesuffix(
            "?image_process=3Dformat,webp/quality,Q_90")
    # 构造数据项并添加到列表中
...
# 将数据列表保存为JSON文件
output_file = "img_src_export.operator.json"
